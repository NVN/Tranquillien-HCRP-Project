...
void Player::Update( uint32 p_time )
{
	...
	//---------- Frakce 70 po splneni questu s ID 2 ----
	...
	setFaction(87);}
       ...
       setFaction(getFactionForRace(getRace()));
		//---------- only arathi -----------
    if(m_mimoZonu){
       if (m_mimoZonuTimer == 0){
         if(GetZoneId() != 45){
           TeleportTo(0, -1628.0f, -3883.0f, 178.88f, 1.8); 
           GetSession()->SendNotification("Byl jsi teleportovan ze zakazane zony");
         }
         m_mimoZonu = false;
       }else m_mimoZonuTimer -= 1;
    }
//---------------------------------
...
void Player::UpdateZone(uint32 newZone, uint32 newArea)
{
    ...
    if((zone->flags & AREA_FLAG_SANCTUARY) || (GetAreaId() == 336) || (GetAreaId() == 313) || (GetAreaId() == 317))                  // in sanctuary
    {
    ...
    {
        if ((HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING)) || (GetAreaId() == 324) || (GetAreaId() == 321))     // but resting (walk from city or maybe in tavern or leave tavern recently)
        {
	...
	//-------Only arathi--------
   if (!isGameMaster() && (GetMapId() != 0 || newZone != 45)){
       GetSession()->SendNotification("Jsi ve spatne zone, jdi zpet, nebo budes teleportovan!");
       m_mimoZonuTimer = 100;
       m_mimoZonu = true;
   }
//--------------------------
...
//>>>>>>> 72002ca29b41fb2fc2ee5a4fc05b61aaa6ddda62:src/game/Player.cpp
                    }
